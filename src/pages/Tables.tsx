...
const Tables = () => {
  ...
  //   const localStudyName = isLocalStudy ? studyIdOrName as AvailableLocalStudy : null;
  ...
  //   const currentStudy: StudyData = apiStudy || (localStudy ? {
  //     ...localStudy.metadata,
  ...
  //         <p>Por favor especifique un ID de estudio válido o nombre de estudio local.</p>
  //         <p>Estudios locales disponibles: {Object.keys(LOCAL_STUDIES).join(', ')}</p>
  //       </div>
  //     );
  //   }
  if (!apiStudy || !apiFilters) {
    return (
      <div style={{ padding: "20px", textAlign: "center" }}>
        <h3>Estudio no encontrado</h3>
      </div>
  ...
  return (
    <div style={{ padding: "16px" }}>
      <AnthropometricTable
              ...
              // start_date={currentStudy.start_date as String}
              ...
              // )}
              percentilesList={apiFilters.percentiles || []}
              age_ranges={(apiFilters.ageRanges || []).join(",")}
              size={apiStudy.size}
              location={`${apiStudy.country} / ${apiStudy.location}`}
              tableTitle={apiStudy.name}
              description={apiStudy.description}
              start_date={apiStudy.start_date}
              end_date={apiStudy.end_date}
              classification={apiStudy.classification}
              gender={apiFilters.gender || ""}
              dimensions={apiFilters.dimensions || []} data={[]} loading={false} error={null}      />
    </div>
  );
};
