...
const PersonFormDialog: React.FC<PersonFormDialogProps> = ({
  ...
  useEffect(() => {
    if (mode === "edit" && personData) {
      ...
      // Prellenar las mediciones
      const initialMeasurements: Record<string, number | null> = {};
      personData.measurements.forEach((measurement) => {
        initialMeasurements[measurement.dimension] = measurement.value;
      });
      setMeasurements(initialMeasurements);

      // Prellenar la posición de medición (si todas las mediciones tienen la misma posición)
      if (personData.measurements.length > 0) {
        setMeasurementPosition(personData.measurements[0].position);
      }
    } else {
