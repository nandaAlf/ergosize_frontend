...
const PercentilesLineChart: React.FC<PercentilesLineChartProps> = ({
  ...
  const chartSeries = series.map(serie => ({
    ...
    connectNulls: true,
    showMark: true,
    color: serie.id === 'p3' ? '#1f77b4' : 
           serie.id === 'p15' ? '#ff7f0e' : 
           serie.id === 'p50' ? '#2ca02c' : 
           serie.id === 'p85' ? '#d62728' : 
           serie.id === 'p97' ? '#9467bd' : '#8c564b'
  }));
    ...
    <Box sx={{ 
      ...
      <Box sx={{ 
        ...
        <Box mt={4}>
          <Typography variant="body1" sx={{ mb: 2 }}>
            Este gráfico muestra la evolución de diferentes percentiles de peso infantil a lo largo de los primeros 15 meses de vida.
          </Typography>
      ...
      <Box sx={{ 
        ...
        <ul style={{ paddingLeft: 20, color: '#555' }}>
          <li>El <strong style={{ color: '#2ca02c' }}>percentil 50 (P50)</strong> representa el valor medio</li>
          <li>Los percentiles <strong style={{ color: '#1f77b4' }}>P3</strong> y <strong style={{ color: '#9467bd' }}>P97</strong> muestran los límites inferior y superior de la distribución normal</li>
          <li>El área entre <strong style={{ color: '#ff7f0e' }}>P15</strong> y <strong style={{ color: '#d62728' }}>P85</strong> representa la mayoría de la población</li>
          <li>Este tipo de gráficos es esencial para el seguimiento del crecimiento infantil</li>
        </ul>
      </Box>
    </Box>
  );
